...
class NewsRenderer:
    ...
    def getNews(parent, news_box):
        ...
        try:
            ...
            # Connect signals
            news_worker.news_fetched.connect(
                lambda items: NewsRenderer._update_news(news_box, items)
            )
            news_worker.error.connect(
                lambda error: NewsRenderer._handle_error(news_box, error)
            )
    ...
    def _update_news(news_box, news_items):
        ...
        news_html = (
            "<span style='font-size: 16px;'><b>Latest WiiLink News</b></span><br><br>"
        )
